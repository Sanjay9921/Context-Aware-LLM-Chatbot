...
def process_batch(batch_df, model, iteration, results_df, context, generate_answer_fn, delay=10):
    ...
    # Baseline
    result = generate_and_score_response(baseline_row["question"], model, expected_keywords, generate_answer_fn, context=context)
    result.update({
        "iteration": iteration,
        "model": model,
    ...
    for i, row in variation_rows.iterrows():
        variation_result = generate_and_score_response(row["question"], model, expected_keywords,
                                                       generate_answer_fn, reference_response, context=context)
